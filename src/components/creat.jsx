...
const Create = () => {
  ...
  // Hàm lưu ghi chú (không cần loadSavedData nữa)
  const handleSave = async () => {
    if (!content.trim()) {
      toast.error("Nội dung ghi chú trống bà ơi!");
      return;
    }

    try {
      const imageUrls = await uploadImagesAndGetURLs(); // 📸 upload ảnh trước khi lưu note

      const noteData = {
        text: content,
        images: imageUrls, // ✅ thêm ảnh vô Firebase luôn
        createdAt: Timestamp.now(),
      };

      const docRef = await addDoc(collection(db, "notes"), noteData);
      console.log("✅ Ghi chú đã lưu với ID:", docRef.id);
      toast.success("Đã lưu vào Firebase!");
      setContent("");
      setImages([]); // xoá ảnh khỏi state
      navigate("/meowstudy/all-notes");
    } catch (error) {
      console.error("🔥 Lỗi khi lưu Firebase:", error);
      toast.error("Không lưu được, thử lại nhen!");
    }
  };
  ...
  const handleImageUpload = (event) => {
    ...
    setImages([...images, ...files]); // ✅ Lưu file gốc thay vì URL.createObjectURL
  };
      ...
      <div className="bg-[url('/photo/Arknights-1.jpg')] bg-cover bg-center bg-no-repeat h-500 min-h-screen ">
        <div className="navbar w-full bg-yellow-50 text-gray-800 shadow-md px-6 py-2 flex justify-between items-center mt-[-2%]">
          <div className="flex-1 flex items-center gap-6">
            <button
              onClick={() => navigate("/meowstudy/")}
              className="text-gray-800 font-bold text-2xl font-sans ml-30 cursor-pointer"
            >
              NOTE MEOW FOR STUDY
            </button>
            <button
              onClick={() => navigate("/meowstudy/all-notes")}
              className="text-gray-800 font-semibold text-xl font-sans ml-30 cursor-pointer rounded-md p-2"
            >
              All Notes
            </button>
            <button
              onClick={() => navigate("/meowstudy/categories")}
              className="text-gray-800 font-semibold text-xl font-sans ml-30 cursor-pointer rounded-md p-2"
            >
              Categories
            </button>
            <button
              onClick={() => navigate("/meowstudy/about-us")}
              className="text-gray-800 font-semibold text-xl font-sans ml-30 cursor-pointer rounded-md p-2"
            >
              About Us
            </button>
          ...
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="Search"
              className="bg-yellow-100 input input-bordered w-24 md:w-auto"
            />
            <div className="dropdown dropdown-end">
              <div
                tabIndex={0}
                role="button"
                className="btn btn-ghost btn-circle avatar bg-yellow-50"
              >
              ...
              >
                <div className="px-4 py-3 border-b border-yellow-200 bg-white bg-opacity-60 rounded-t-xl">
                  ...
                  <button
                    onClick={() => navigate("/meowstudy/profile")}
                ...
                <li>
                  <ul className="text-sm text-gray-700 divide-y divide-yellow-100">
                    {["Settings", "Help", "Accessibility", "Feedback"].map(
                      (item) => (
                        <li
                          key={item}
                          className="px-4 py-2 hover:bg-yellow-50 transition duration-200 cursor-pointer"
                        >
                          {item}
                        </li>
                      )
          ...
          <div className="max-w-3xl mx-auto bg-yellow-30 p-6 rounded-lg shadow-lg border-4 border-yellow-300">
            ...
            <div className="flex justify-between mb-4">
              <input
                type="text"
                placeholder="DATE"
                className="bg-white text-gray-800 px-3 py-2 rounded border border-yellow-40"
              />
              <input
                type="text"
                placeholder="TITLE"
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
              {images.map((file, index) => (
                <div key={index} className="relative">
                  <img
                    src={URL.createObjectURL(file)} // ✅ Chỉ dùng để preview
                    alt="Uploaded"
                    className="rounded-lg shadow-md border border-yellow-30"
                  />
                  <button
                    className="absolute top-1 right-1 bg-red-50 px-2 py-1 text-sm text-gray-800 rounded"
